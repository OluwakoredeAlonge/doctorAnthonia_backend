...
<head>
  ...
  <meta property="og:site_name" content="Dr. Anthonia Yemisi Soje" />
  @if($blogPost->featured_image)<meta property="og:image" content="{{ $blogPost->featured_image }}" />@endif
  @if($blogPost->published_at)<meta property="article:published_time" content="{{ $blogPost->published_at->toIso8601String() }}" />@endif
  <meta property="article:author" content="Dr. Anthonia Yemisi Soje" />
  <!-- Twitter Card -->
  <meta name="twitter:card" content="summary_large_image" />
  <meta name="twitter:title" content="{{ $blogPost->title }}" />
  <meta name="twitter:description" content="{{ $blogPost->excerpt }}" />
  @if($blogPost->featured_image)<meta name="twitter:image" content="{{ $blogPost->featured_image }}" />@endif
  <!-- JSON-LD: Article -->
  <script type="application/ld+json">
  {
    "@context": "https://schema.org",
    "@type": "Article",
    "headline": {{ json_encode($blogPost->title) }},
    "description": {{ json_encode($blogPost->excerpt) }},
    "url": "{{ route('blog.show', $blogPost) }}",
    @if($blogPost->featured_image)"image": "{{ $blogPost->featured_image }}",@endif
    @if($blogPost->published_at)"datePublished": "{{ $blogPost->published_at->toIso8601String() }}",@endif
